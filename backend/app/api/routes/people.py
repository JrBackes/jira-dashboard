...
def person_workload(person_id: int, project: str | None = None, db: Session = Depends(get_db)):
    if person_metrics.get_person(db, person_id) is None:
        raise HTTPException(status_code=404, detail="Pessoa não encontrada")
    return person_metrics.person_workload(db, person_id, project_key=project)
