...
- Carga de trabalho (`person_workload`) e contagem por status da sprint (`sprint_status_counts`) agrupam pelo `status` granular do time (Backlog/To Do/In Progress/Is Blocked/To Test/Testing/To Review/Review/Deploy para prod), não mais pelas 3 categorias genéricas do Jira — ordenados pela ordem lógica do fluxo (`app/services/status_order.py`). Ver taxonomia completa em `AGENTS.md`.
