...
- **Calendário de deploy (confirmado com o usuário, 2026-07-18):** deploy para produção roda até quinta à tarde (sexta só em caso de emergência); o que sobra é deployado na segunda de manhã da semana seguinte. Um item em "Deploy para prod" já está com o trabalho do time concluído, mas ainda aguarda essa janela. **"To Review"/"Review", "Waiting Store Approval" e "Reviewed" já estão em produção** (confirmado com o usuário) — não têm relação nenhuma com a janela de deploy, só estão em fases posteriores de revisão/aprovação pós-deploy. Ficar parado em qualquer um desses status por dias é esperado, não sinal de atraso real. Usado em `sprint_risk_items` (`app/services/sprint_metrics.py`, campos `awaiting_deploy_count` vs `in_production_count`) pra não marcar esses itens como "em risco".
